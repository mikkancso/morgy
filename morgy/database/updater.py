    ...
    def update_db(self, directory, priority):
        for dirpath, dirnames, filenames in os.walk(directory):
            ...
            for name in filtered_filenames:
                (artist, year, album, cd_number, number, title) = self.detail_fetcher.fetch_detail(dirpath, name[:-4])
                path = os.path.join(dirpath, name)
                self.db.add_detail_row(path, artist, year, album, cd_number, number, title, priority)
...
def main():
    ...
    parser.add_argument('-p', '--priority', type=int, choices=range(1, 11), default=10, help='The priority to add to new music.')
